...
def resolve_target(scope: str, repo_root: str, target_root: str) -> Path:
    if scope == "repo":
        ...
        return Path(repo_root).expanduser().resolve() / ".cursor" / "skills"
    if scope == "user":
        return home_dir() / ".cursor" / "skills"
    if scope == "custom":
        if not target_root:
...
def install(source_root: Path, target_root: Path, dry_run: bool, backup_dir: Path | None = None, repo_root: Path | None = None) -> dict[str, Any]:
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Install CodexAI skills into Cursor skill directories.")
    parser.add_argument("--source", default="", help="Source skills directory. Defaults to this repo's skills root.")
    parser.add_argument("--scope", choices=("repo", "user", "custom"), default="user")
...
def main() -> int:
    ...
    try:
        ...
        payload = install(source_root, target_root, dry_run=dry_run, backup_dir=backup_dir, repo_root=repo_root)
    ...
    if args.format == "text":
        print(
            f"{payload['status']}: scope={payload['scope']} target={payload['cursor_target']} "
