...
- macOS/Linux:
  `python "<SKILLS_ROOT>/codex-project-memory/scripts/build_knowledge_index.py" --project-root <path>`
- Output:
  `.codex/knowledge/index.json`, `.codex/knowledge/INDEX.md`, and `.codex/knowledge/knowledge-graph.json`. Add `--html` (or `--watch`/`--serve`) to also write `.codex/knowledge/index.html`.
- Query:
